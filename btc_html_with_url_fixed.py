#!/usr/bin/env python3
"""生成包含 Netlify URL 的 BTC HTML 报告"""
...
HTML_OUTPUT = f"{WORKSPACE}/btc_report_enhanced.html"
NETLIFY_URL = "https://btc-tracker-report.netlify.app"
TRADES_FILE = f"{WORKSPACE}/btc_trades.json"
...
def read_latest_reports():
    ...
    # 检查市场分析报告
    market_report_files = sorted([os.path.join(REPORTS_DIR, f) for f in os.listdir(REPORTS_DIR) if f.startswith("btc_report_")])
    if market_report_files:
        with open(market_report_files[-1], 'r', encoding='utf-8') as f:
            reports['market'] = f.read()

    # 检查回测报告
    backtest_report_files = sorted([os.path.join(REPORTS_DIR, f) for f in os.listdir(REPORTS_DIR) if f.startswith("btc_backtest_report_")])
...
def generate_enhanced_html(reports, backtest_metrics):
    """生成增强版 HTML 报告，确保包含 Netlify URL"""
    ...
    # 使用单行字符串，避免换行符破坏 HTML
    total_return_class = "positive" if total_return > 0 else "negative"
    annual_return_class = "positive" if total_return > 0 else "negative"
    win_rate_class = "positive" if win_rate > 50 else "negative"
    profit_loss_ratio_class = "positive" if profit_loss_ratio > 0 else "negative"

    total_return_html = f"{total_return:.2f}%"
    annual_return_html = f"{total_return * 100:.2f}%"
    win_rate_html = f"{win_rate:.2f}%"
    profit_loss_ratio_html = f"{profit_loss_ratio:.2f}%"
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="metric-grid">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value {total_return_class}">{total_return_html}</div>
                </div>
                <div class="metric-card">
                    <div class="metric-label">年化收益率</div>
                    <div class="metric-value {annual_return_class}">{annual_return_html}</div>
                </div>
                <div class="metric-card">
                    <div class="metric-label">胜率</div>
                    <div class="metric-value {win_rate_class}">{win_rate_html}</div>
                </div>
                <div class="metric-card">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value positive">{win_count}</div>
                </div>
                <div class="metric-card">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value positive">${max_capital:,.0f}</div>
                </div>
                <div class="metric-card">
                    <div class="metric-label">最低净值</div>
                    <div class="metric-value negative">${min_capital:,.0f}</div>
                </div>
                <div class="metric-card">
                    <div class="metric-label">回撤持续时间</div>
                    <div class="metric-value">{max_drawdown_duration_hours:.1f} 小时</div>
                </div>
                <div class="metric-card">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value {profit_loss_ratio_class}">{profit_loss_ratio_html}</div>
                </div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <div class="content-box">
                <div class="pre-formatted">{market_content}</div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <div class="content-box">
                <div class="pre-formatted">{backtest_content}</div>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            <p>🔄 自动生成时间：{now_str}</p>
            <p>🌐 访问网址： <a href="{NETLIFY_URL}" target="_blank" style="color: #667eea;">{NETLIFY_URL}</a></p>
        </div>
    </div>
...
def main():
    """主函数"""
    print(f"[{datetime.now()}] 开始生成最终版 HTML 报告...")
    ...
    print(f"\n{'='*60}")
    print(f"  📊 文件: {HTML_OUTPUT}")
    print(f"  🌐 访问网址: {NETLIFY_URL}")
